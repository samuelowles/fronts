...
* Statistics are part of the output, not a debug flag. ``plan`` prints
  ...
  (see ``PlanResult`` in ``fronts.protocols``).
"""
...
from fronts.config import FrontsConfig, default_config
from fronts.config import load as load_config
...
from fronts.cwm.llm import AnthropicClient, OpenAIClient
...
from fronts.cwm.refine import best as best_node
...
from fronts.game.action_space import ActionCodec, ActionDecodeError, restamp_unique
from fronts.game.legality import LegalityContext, LegalityEngine
from fronts.game.payoff import allowable_cac, ltv
...
from fronts.protocols import CodeWorldModel, StateInference
...
# `fronts --seed 5 plan` and `fronts plan --seed 5`.
# ---------------------------------------------------------------------------
...
def _config_from_args(args: argparse.Namespace) -> FrontsConfig:
...
def _read_plan(config: FrontsConfig) -> list[Move]:
    path = config.paths.plan
    if not path.exists():
        raise FileNotFoundError(
            f"no plan at {path}; run `fronts plan` first (or set paths.plan "
            "in your config)"
        )
...
def _client_from_env(config: SynthConfig) -> AnthropicClient | OpenAIClient:
    ...
    raise RuntimeError(
        ...
        "OPENAI_API_KEY (pip install \"fronts[llm]\" for the SDKs)"
    )
...
def _cmd_synth(args: argparse.Namespace) -> int:
    ...
    # (PAPER.md s6). Written beside the model so a later `fronts plan` in a
    ...
    if inference_source is not None:
        ...
        print("  inference:    synthesis failed; `fronts plan` will run open-loop")
    print(f"  llm calls:    {2 + sum(node.refinements for node in tree.nodes)}")
    print("  next: `fronts accuracy` before trusting `fronts plan`")
    return EXIT_OK
...
def _cmd_accuracy(args: argparse.Namespace) -> int:
    ...
    if sampler is None:
        inference_note = (
            ...
            "(fronts synth writes one beside the model)."
        )
    else:
        inference_note = (
...
def _cmd_arena(args: argparse.Namespace) -> int:
    ...
    if len(hosts) == 1:
        print(
            ...
            "models; run `fronts synth` and re-run to reject bad models."
        )
    return EXIT_OK
...
def app(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"fronts: {exc}", file=sys.stderr)
        return EXIT_ERROR
    except (OSError, ValueError, RuntimeError, ImportError, KeyError) as exc:
        print(f"fronts: {type(exc).__name__}: {exc}", file=sys.stderr)
        return EXIT_ERROR
