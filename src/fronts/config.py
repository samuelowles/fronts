...
from fronts.adapters.composio_io import ComposioConfig
from fronts.game.legality import OperatorPolicy
from fronts.game.payoff import Economics
from fronts.game.types import Format

__all__ = ["Paths", "FrontsConfig", "load", "example", "default_config"]
...
def default_config() -> FrontsConfig:
...
def _toml_parser() -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            "loading a fronts config requires the standard library's TOML "
...
def load(path: str | Path) -> FrontsConfig:
...
def example() -> str:
    ...
    ``fronts.example.toml`` (a test asserts the identity, so the shipped
